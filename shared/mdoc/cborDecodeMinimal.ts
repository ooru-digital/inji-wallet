...
/**
 * True when any BLE transfer-method row carries map key 21.
 *
 * Readers built on Multipaz map key 21 to `MdocConnectionMethodBle.peripheralServerModePsm` — an
 * **L2CAP PSM**. A holder that serves only GATT must not advertise one, or the reader can loop
 * trying to open L2CAP on that port and never fall back. Used to invalidate engagements that were
 * persisted before that was understood, since the row *count* is unchanged and the cache would
 * otherwise happily replay the old QR.
 */
export function deviceEngagementAdvertisesBlePsm(
  deviceEngagementCbor: Uint8Array,
): boolean {
  try {
    const r = new Reader(deviceEngagementCbor);
    ...
    for (const row of tm) {
      ...
      const ble = row[2];
      if (ble instanceof Map && typeof ble.get(21) === 'number') {
        return true;
      }
    }
    return false;
  } catch {
    // An engagement we cannot parse is regenerated by the caller's other checks anyway.
    return false;
  }
}
